...
class PhaseThreeService {
    ...
    public function sale(array $d, ?Sale $m=null): Sale { return DB::transaction(function()use($d,$m){
        $d['total_amount']=round((int)$d['quantity']*(float)$d['weight']*(float)$d['price'],2);
        $d['paid_amount']=round((float)($d['paid_amount']??0),2);
        if($d['paid_amount']>$d['total_amount']) throw ValidationException::withMessages(['paid_amount'=>['Paid amount cannot exceed total amount.']]);
